...
const ConvertSheet = React.forwardRef((props, ref) => {
  ...
  const available = currencies.filter(c => c.id !== primaryCurrency)
      ...
      {available.map(c =>
        <TouchableOpacity
          ...
          onPress={() => {
            ref.current.close()
            setTimeout(() => {
              navigation.navigate('ConvertScreen', { currency1: primaryCurrency, currency2: c.id })
            }, 500)
          }}
        >
          <>

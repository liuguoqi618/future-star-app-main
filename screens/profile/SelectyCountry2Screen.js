...
import { countries } from '../../utils/verification'
...
export default function SelectCountry2Screen({ navigation, route }) {
  const { t } = useTranslation()

  const { value, setValue } = route.params

  const currentIndex = value ? countries.findIndex(o => o.country === value.country) : -1
  const currentValue = currentIndex >= 0 ? countries[currentIndex] : null
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={countries.filter(c => t(c.label).toLocaleLowerCase().includes(input.toLocaleLowerCase()))}
        renderItem={({ item }) =>
          <MemoizedItem
            item={item}
